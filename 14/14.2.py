...
def sand_simulation(board):
    board = copy.deepcopy(board)
    floor = max([point.y for point in board.keys()]) + 2

    sand = source

    while True:        
        if board.get(sand.add(0, 1), '.') not in ["#", "o"] and sand.y + 1 < floor:
            sand = sand.add(0, 1)
            continue
        elif board.get(sand.add(-1, 1), '.') not in ["#", "o"] and sand.y + 1 < floor:
            sand = sand.add(-1, 1)
            continue
        elif board.get(sand.add(1, 1), '.') not in ["#", "o"] and sand.y + 1 < floor:
            sand = sand.add(1, 1)
...
res_board = sand_simulation(board)
